...
def create_pq_config(sugar: str, run_dir: Path) -> None:
    ...
    pq_config = {
        "InputFolders": [
            "../../debug/mv_altlocs/all_altlocs_for_mv/",
        ],
        "Queries": [],
...
if __name__ == "__main__":
    ...
    run_dir = Path(f"../../debug/mv_altlocs/pq_{datetime.now().strftime('%Y-%m-%dT%H-%M-%S')}")
    Path(f"../../debug/mv_altlocs/pq_{datetime.now().strftime('%Y-%m-%dT%H-%M-%S')}").mkdir(exist_ok=True, parents=True)
