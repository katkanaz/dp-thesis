...
def tanglegram(a, b, n_data, labelsA=None, labelsB=None, edges=None, sort=True, figsize=(30, 30),
    ...
    colors = {
        1: '#e6194b', 2: '#3cb44b', 3: '#ffe119', 4: '#4363d8', 5: '#f58231', 6: '#911eb4',
        7: '#46f0f0', 8: '#f032e6', 9: '#bcf60c', 10: '#fabebe', 11: '#008080', 12: '#e6beff',
    ...
    for i, i12 in enumerate(link1[:,:2].astype(int)):
        c1, c2 = (link_cols[x] if x > len(link1) else link_cols[x]
            for x in i12)
        link_cols[i+1+len(link1)] = c1 if c1 == c2 else dflt_col


    # Compute and plot left dendrogram.
    ax1 = fig.add_axes([0.05, 0.1, 0.25, 0.86])
    ax1.set_title ("super", fontsize=50)
    Z1 = sclust.hierarchy.dendrogram(link1, color_threshold=6.1, orientation='left', labels=labels, link_color_func=lambda x: link_cols[x], **dend_kwargs)

    # Compute and plot right dendrogram.
    ax2 = fig.add_axes([0.7, 0.1, 0.25, 0.86])
    ax2.set_title("align", fontsize=50)
    Z2 = sclust.hierarchy.dendrogram(link2, orientation='right', labels=labels, **dend_kwargs)

    # Generate middle plot with connecting lines
    ax3 = fig.add_axes([0.32, 0.1, 0.36, 0.86])
    ax3.axis('off')
    ax3.set_xlim((0, 1))
    ...
    for i, e in enumerate(edges):
        ...
        if not color_by_diff:
            #labels_super = sclust.hierarchy.fcluster(Z1, t=22, criterion='maxclust')
            if len(e) == 3:
                c = e[2]
            else:
                c = 'black'
        else:
            for cluster, leafs in clusters.items():
                if e[0] in leafs:
                    c = colors[int(cluster)]

        ax3.plot([0, 1], [coords_l, coords_r], '-', linewidth=5, c=c)
