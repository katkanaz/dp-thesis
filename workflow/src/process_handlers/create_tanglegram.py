...
def create_tanglegram(sugar: str, number: int, method: str, config: Config, perform_align: bool) -> None:
    """
    Call tanglegram function from an external script, which is modified to show the data as needed for this analysis.

    :param sugar: The sugar for which the representative surrounding is being defined
    :param number: The number of created clusters 
    :param method: Chosen clustering method for matrix computation
    ...
    Z_super = sph.linkage(D_super, method=method)
    Z_align = sph.linkage(D_align, method=method)

    # Total number of surroundings for given sugar
    n_data = Z_super.shape[0] + 1

    path_to_file = config.clusters_dir / "super" / f"{number}_{method}_all_clusters.json"
...
if __name__ == "__main__":
    ...
    create_tanglegram(args.sugar, args.number, args.method, config, args.perform_align)
